...
def generate_random_id(timestamp, prefix_length=8):
    """Generate a random ID with timestamp and random characters"""
    # Generate random string of letters and digits
    random_str = ''.join(random.choices(string.ascii_letters + string.digits, k=prefix_length))
...
def update_json_with_leads(headers, rows, account_id):
    ...
    try:
        ...
        for row in rows:
            ...
            # Generate a random object ID for the task
            random_object_id = generate_random_id(current_time)
                ...
                "tasks": [
                    {
                        ...
                        "objectID": random_object_id,  # Random objectID instead of fixed format
                    }
                ],
